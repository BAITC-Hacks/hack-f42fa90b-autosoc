...
const approvalOpen = new WeakMap();
let formResultsStale = false;
...
const ui = {
  ru: {
    ...
    initialTitle: "Начните с параметров события", initialBody: "Заполните форму или попробуйте один из проверенных сценариев. Результаты появятся здесь.", loadingOptions: "Загружаем варианты из каталога…", ready: "Форма готова. Запустите подбор, чтобы увидеть результаты.", optionsErrorStatus: "Не удалось загрузить варианты для формы.", optionsError: "Связь с сервисом не установлена. Проверьте, что приложение запущено, и повторите загрузку.", edited: "Параметры формы изменены. Показанные карточки относятся к предыдущему запросу; запустите новый подбор.", optionsPending: "Варианты каталога ещё не загружены.", fixForm: "Исправьте поля формы и повторите запрос.", formInvalid: "Проверьте обязательные поля, бюджет, длительность и дату в пределах календаря.", matching: "Проверяем условия и занятость. Предыдущие результаты убраны.", rejectedRequest: "Запрос отклонён проверкой данных.", completed: "Подбор завершён за {seconds} с.", matchErrorStatus: "Не удалось получить результаты.", matchError: "Ответ сервиса не получен. Проверьте соединение и повторите запрос.",
    ...
    shareChoice: "Согласовать выбор", approvalTitle: "Сводка для обсуждения", approvalHelp: "Локальный текст: никому не отправляется. Проверьте условия перед решением.", approvalTextLabel: "Текст сводки; можно выделить и скопировать вручную", approvalCopy: "Скопировать текст", approvalClose: "Закрыть", approvalCopied: "Скопировано", approvalCopyFailed: "Автоматическое копирование недоступно. Выделите текст и скопируйте вручную.", approvalProfile: "Профиль: {name} (ID {id})", approvalConditions: "Условия показанного подбора:", approvalReasons: "Подтверждённые причины:", approvalReasonFormat: "Профиль категории «{category}» в городе {city} поддерживает формат «{format}».", approvalReasonBudget: "Цена от {price} не превышает бюджет {budget}.", approvalPrice: "Цена от {price}; окончательную смету нужно уточнить.", approvalCalendar: "По календарю каталога не отмечен занятым; доступность нужно подтвердить.", approvalAvailabilityUnknown: "Занятость на эту дату неизвестна; доступность нужно подтвердить.", approvalDataNotes: "Пометки о данных:", approvalSynthetic: "Профиль помечен как синтетический.", approvalCityImputed: "Город восстановлен в данных.", approvalPriceImputed: "Цена восстановлена в данных.", approvalQuestions: "Вопросы перед решением:", approvalQuestionPrice: "Какова итоговая стоимость и что входит в услугу?", approvalQuestionDate: "Можете подтвердить доступность на выбранную дату?", approvalQuestionDuration: "Какова длительность услуги? В профиле часы не указаны.",
  },
  kk: {
    ...
    shareChoice: "Таңдауды келісу", approvalTitle: "Талқылауға арналған түйін", approvalHelp: "Жергілікті мәтін: ешкімге жіберілмейді. Шешім алдында шарттарды тексеріңіз.", approvalTextLabel: "Түйін мәтіні; қолмен таңдап көшіруге болады", approvalCopy: "Мәтінді көшіру", approvalClose: "Жабу", approvalCopied: "Көшірілді", approvalCopyFailed: "Автоматты көшіру мүмкін емес. Мәтінді таңдап, қолмен көшіріңіз.", approvalProfile: "Профиль: {name} (ID {id})", approvalConditions: "Көрсетілген іздеудің шарттары:", approvalReasons: "Дерекпен расталған себептер:", approvalReasonFormat: "{city} қаласындағы «{category}» санатындағы профиль «{format}» форматына сай.", approvalReasonBudget: "{price} бастап баға {budget} бюджетінен аспайды.", approvalPrice: "Бағасы {price} бастап; түпкілікті сметаны нақтылау керек.", approvalCalendar: "Каталог күнтізбесінде бос емес деп белгіленбеген; қолжетімділігін растау керек.", approvalAvailabilityUnknown: "Бұл күндегі бос еместігі белгісіз; қолжетімділігін растау керек.", approvalDataNotes: "Дерек туралы белгілер:", approvalSynthetic: "Профиль синтетикалық деп белгіленген.", approvalCityImputed: "Қала деректе қалпына келтірілген.", approvalPriceImputed: "Баға деректе қалпына келтірілген.", approvalQuestions: "Шешім алдындағы сұрақтар:", approvalQuestionPrice: "Қорытынды құны қандай және қызметке не кіреді?", approvalQuestionDate: "Таңдалған күні қолжетімді екеніңізді растай аласыз ба?", approvalQuestionDuration: "Қызмет ұзақтығы қанша? Профильде сағат көрсетілмеген.",
  },
  en: {
    ...
    initialTitle: "Start with your event requirements", initialBody: "Complete the form or try a verified scenario. Results will appear here.", loadingOptions: "Loading catalog options…", ready: "The form is ready. Run a search to see results.", optionsErrorStatus: "Could not load form options.", optionsError: "Could not reach the service. Check that the app is running and retry.", edited: "Form requirements changed. Shown cards belong to the previous request; run matching again.", optionsPending: "Catalog options have not loaded yet.", fixForm: "Correct the form fields and retry.", formInvalid: "Check required fields, budget, duration, and a date within the calendar.", matching: "Checking requirements and calendar. Previous results have been hidden.", rejectedRequest: "The request failed validation.", completed: "Matching completed in {seconds} s.", matchErrorStatus: "Could not get results.", matchError: "No response from the service. Check the connection and retry.",
    ...
    shareChoice: "Share for approval", approvalTitle: "Discussion preview", approvalHelp: "Local text only: nothing is sent. Check the terms before deciding.", approvalTextLabel: "Discussion text; select and copy manually if needed", approvalCopy: "Copy text", approvalClose: "Close", approvalCopied: "Copied", approvalCopyFailed: "Automatic copying is unavailable. Select the text and copy it manually.", approvalProfile: "Profile: {name} (ID {id})", approvalConditions: "Requirements of the displayed match", approvalReasons: "Verified reasons", approvalReasonFormat: "This {category} profile in {city} supports the {format} format.", approvalReasonBudget: "The starting price of {price} is within the {budget} budget.", approvalPrice: "Starting price {price}; confirm the final quote.", approvalCalendar: "Not marked busy in the catalog calendar; availability needs confirmation.", approvalAvailabilityUnknown: "Availability for this date is unknown; please confirm it.", approvalDataNotes: "Data notes", approvalSynthetic: "This profile is marked synthetic.", approvalCityImputed: "The city was imputed in the data.", approvalPriceImputed: "The price was imputed in the data.", approvalQuestions: "Questions before deciding", approvalQuestionPrice: "What is the final price and what does the service include?", approvalQuestionDate: "Can you confirm availability on the selected date?", approvalQuestionDuration: "How long does the service last? Hours are not stated in the profile.",
  },
};
...
function onFormEdited() {
  ...
  stopPreviousRequest();
  clearError();
  if (renderedResults.has(resultsElement)) {
    formResultsStale = true;
    resultsElement.querySelector(".stale-note")?.remove();
    resultsElement.prepend(element("p", "stale-note", t("edited")));
  } else {
...
async function requestMatch() {
  stopPreviousRequest();
  clearError();
  formResultsStale = false;
  approvalOpen.delete(resultsElement);
...
function renderApproval(card, query, target, index) {
  if (!query) return null;
  const snapshot = { ...query };
  const wrapper = element("div", "approval-area");
  const toggle = element("button", "approval-toggle", t("shareChoice"));
  toggle.type = "button";
  const panel = element("section", "approval-preview");
  panel.id = `approval-${target.id}-${index}`;
  panel.setAttribute("aria-label", t("approvalTitle"));
  panel.hidden = approvalOpen.get(target) !== card.id;
  toggle.setAttribute("aria-controls", panel.id);
  toggle.setAttribute("aria-expanded", String(!panel.hidden));
  const title = element("h4", "", t("approvalTitle"));
  const help = element("p", "", t("approvalHelp"));
  const preview = element("textarea", "approval-text");
  preview.readOnly = true;
  preview.rows = 15;
  preview.setAttribute("aria-label", t("approvalTextLabel"));
  preview.value = window.buildApprovalText(card, snapshot, { t, catalogLabel, money, dateLabel });
  const controls = element("div", "approval-controls");
  const copy = element("button", "approval-copy", t("approvalCopy"));
  copy.type = "button";
  const close = element("button", "approval-close", t("approvalClose"));
  close.type = "button";
  const status = element("p", "approval-status");
  status.setAttribute("role", "status");
  status.setAttribute("aria-live", "polite");
  const closePanel = () => {
    approvalOpen.delete(target);
    panel.hidden = true;
  ...
  toggle.addEventListener("click", () => {
    ...
    for (const other of target.querySelectorAll(".approval-preview")) other.hidden = true;
    for (const other of target.querySelectorAll(".approval-toggle")) other.setAttribute("aria-expanded", "false");
    approvalOpen.set(target, card.id);
    panel.hidden = false;
  ...
  copy.addEventListener("click", async () => {
    ...
    if (await window.copyApprovalText(preview.value, clipboardWrite)) {
      status.textContent = t("approvalCopied");
    } else {
      status.textContent = t("approvalCopyFailed");
      preview.focus();
      preview.select();
    }
  });
  ...
  panel.append(title, help, preview, controls, status);
  wrapper.append(toggle, panel);
  return wrapper;
}

function renderCard(card, index, responseLocale, query, target) {
  ...
  const approval = renderApproval(card, query, target, index);
  if (approval) article.append(approval);
  return article;
}
...
function renderResponse(response, target = resultsElement, query = null, responseLocale = locale, preserve = false) {
  ...
  if (!preserve) approvalOpen.delete(target);
  const content = [renderOutcome(response, query, responseLocale)];
  if (target === resultsElement && formResultsStale) content.unshift(element("p", "stale-note", t("edited")));
  ...
  for (const [index, card] of response.cards.slice(0, 3).entries()) content.push(renderCard(card, index, responseLocale, query, target));
